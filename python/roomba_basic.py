...
##################################################################################################################################################################
            ...
            while time.time() - start_time < 20:  # 現在の時刻と開始時刻の差が3秒未満の間 つまり3秒間ループ処理
                ...
                if distance_wayp >=100: # 目標の位置の方位と今のルンバの向きの方位の差が５以上だったら　旋回して角度合わせる，
                    DrivePWM(ser, int(0.15*distance_wayp-80*delta_thita_wayp)+29 , int(0.15*distance_wayp-80*delta_thita_wayp)+29) #差の大きさの変数をそのまま出力にするのが一番よさそう（P制御）
                    print(int(0.15*distance_wayp-80*delta_thita_wayp+29) , int(0.15*distance_wayp-80*delta_thita_wayp+29))
                else:
                    DrivePWM(ser, 0,0)
