...
#################################################################################################################################################################
            ...
            while time.time() - start_time < 2:  # 現在の時刻と開始時刻の差が3秒未満の間 つまり3秒間ループ処理
                ...
                if thita_wayp - now_ang >=5: # 目標の位置の方位と今のルンバの向きの方位の差が５以上だったら　旋回して角度合わせる，
                    DrivePWM(60,0) #差の大きさの変数をそのまま出力にするのが一番よさそう（P制御）
                if thita_wayp - now_ang >=-5: # 目標の位置の方位と今のルンバの向きの方位の差が５以上だったら　旋回して角度合わせる，
                    DrivePWM(0,60) #差の大きさの変数をそのまま出力にするのが一番よさそう（P制御）
