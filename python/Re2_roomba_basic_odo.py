...
def print_function():
    ...
    th = threading.Thread(target = print_encs_value, args=(ser, val))
    th.start()

'''各エンコーダ値取得関数'''

def print_encs_value(ser, val):
    Ser = ser
    Val = val
    Memory = [0, 0, 0, 0, 0]
    init = []
    ...
    while not exit_signal.is_set():
        ...
        # if Val == '1':
        re_x_t = x_t - (0.0000301365 * (time.time() - init_time) + 0.0025251277) / 100
        re_y_t = y_t + (-0.000038282282  * (time.time() - init_time) - 0.001315841409 ) *1
        # re_y_t = y_t - (0.0000301365 * (time.time() - init_time)) / 100
...
def main():
    global ser, Sender, Destination, val
    ...
    '''シリアル通信開始'''
    ser = serial.Serial(RB_PORT, RB_RATE, write_timeout=0)
    ...
    try:
        while True:
            # RecvData = Reciever.recvfrom(12)
            # Data = struct.unpack('fff', RecvData[0])
            # re_Data =int(Data)
